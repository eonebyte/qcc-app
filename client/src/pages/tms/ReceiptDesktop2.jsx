...
import './ReceiptDesktop2.css';

const backEndUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:3200';

const ReceiptDesktop2 = () => {
    ...
    const getRecipientTo = (checkpoinId) => {
        switch (String(checkpoinId)) {
            case '6': return 'DRIVER';
            case '8': return 'DPK';
            case '2': return 'DELIVERY';
            case '10': return 'DELIVERY';
            case '14': return 'FAT';
            default: return 'DPK';
        }
    };
                ...
                const processedData = rawBundles.map(bundle => {
                    const processedShipments = bundle.shipments
                        .map(shipment => ({
                            ...
                            arrived: false,
                            to: getRecipientTo(shipment.checkpoin_id),
                        }))
                        .filter(shipment => {
    ...
    const totalSelectedCount = data.reduce((total, bundle) => total + bundle.shipments.filter(s => s.arrived).length, 0);
    ...
    const executeSubmit = async () => {
        ...
        try {
            // Checkpoint diambil dari shipment pertama di bundle pertama
            const checkpoint = selectedBundlesForSubmit[0].shipments[0].checkpoin_id;

            // PAYLOAD SEKARANG BERISI ARRAY OF BUNDLES
...
export default ReceiptDesktop2;