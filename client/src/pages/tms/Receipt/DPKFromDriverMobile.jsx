            ...
            if (res.data.data && res.data.data.success) {
                ...
                const processedData = rawBundles.map(bundle => {
                    ...
                    return {
                        ...
                        bundleSelected: false, // Untuk checkbox bundle
                    };
        ...
        setDataList(prev => prev.map(bundle => {
            if (bundle.bundleNo === bundleNo) {
                ...
                return { 
                    ...
                    shipments: bundle.shipments.map(s => ({...s, arrived: newStatus})) // Sync 'arrived' prop
                };
            }
            return bundle;
        }));
            ...
            onConfirm: async () => {
                try {
                    ...
                    if (res.data.success) {
                        ...
                        fetchData(); // Refresh data untuk memastikan bundle update
                    } else {
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>
                                {bundle.drivername ? (
                                    <><UserOutline /> {bundle.drivername} • </>
                                ) : ''}
                                {bundle.shipments.length} Docs
                            </div>
                            <div style={{ fontSize: 11, color: '#999' }}>
                                {dayjs(bundle.created).add(7, 'hours').format('DD-MM-YYYY HH:mm')}
                            </div>
                        </div>
                    </div>
                }
