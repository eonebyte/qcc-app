...
const FATFromMKTMobile = () => {
            ...
            if (res.data.data && res.data.data.success) {
                ...
                const processedData = rawBundles.map(bundle => {
                    const processedShipments = bundle.shipments
                        ...
                        .filter(shipment => {
                            // Filter logic: Checkpoint 4 & User ID
            ...
            if (bundle.bundleNo === bundleNo) {
                const updatedShipments = bundle.shipments.map(s => {
                    if (s.key === shipmentKey) {
                        ...
                        if (newCount >= 3) {
                            Toast.show({ content: 'Status di-reset (Unchecked)', icon: 'success' });
        ...
        Dialog.confirm({
            ...
            content: (
                <div style={{ maxHeight: '40vh', overflowY: 'auto' }}>
                    ...
                    <ul style={{ paddingLeft: 20, fontSize: 13, textAlign: 'left', color: '#666' }}>
            ...
            onConfirm: async () => {
                try {
                    ...
                    const res = await axios.post(`${backEndUrl}/receipt/process/fat/from/mkt`, { data: payloadData }, { withCredentials: true });
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: 12, color: '#888' }}>
                                SPP: {bundle.sppno || 'N/A'} • {bundle.shipments.length} Docs
                            </div>
                        </div>
                    </div>
                }
                        ...
                        <Card key={item.key} style={{ marginBottom: 8, borderRadius: 6 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: 13, color: '#666' }}>{item.customer}</div>
    ...
    return (
        <LayoutGlobalMobile title="Receipt from Marketing">
...
export default FATFromMKTMobile;