...
import DPKToDeliveryMobile from "./DPKToDeliveryMobile";

const backEndUrl = import.meta.env.VITE_BACKEND_URL || "http://localhost:3200";

export default function DPKToDelivery() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await fetch(`${backEndUrl}/handover/list/dpk/to/delivery`, {
        credentials: "include",
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      const resp = await fetch(
        `${backEndUrl}/handover/process/dpk/to/delivery`,
        {
          method: "POST",
