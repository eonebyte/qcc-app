...
export default function CheckOutMobile() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await fetch(`${backEndUrl}/handover/list/checkin/customer`, {
        credentials: "include",
      });
      const json = await resp.json();
      const listData = json?.data?.data || [];

      const dataFiltered = listData.filter(
  ...
  const toggleSelectionDropOnly = (record) => {
    ...
    if (isSelected) {
      setSelectedRowsDropOnly(
        selectedRowsDropOnly.filter((r) => r.key !== record.key),
      );
    } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const resp = await fetch(
        `${backEndUrl}/handover/process/driver/to/customer`,
        {
          method: "POST",
  ...
  const handleCancelOk = async () => {
    try {
      ...
      const res = await axios.post(`${backEndUrl}/tms/req/cancel`, payload, {
