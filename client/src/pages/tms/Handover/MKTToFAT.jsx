...
import MKTToFATMobile from "./MKTToFATMobile";
...
export default function MKTToFAT() {
  ...
  const expandedRowRender = (record) => {
    return (
      <div
        style={{ padding: "8px 40px", margin: 0, backgroundColor: "#fafafa" }}
      >
        <Table
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!validDriver) {
        message.error(
          "Semua dokumen dalam grup yang dipilih harus memiliki driverBy yang sama!",
        );
        return;
      }
      if (!validTnkb) {
        message.error(
          "Semua dokumen dalam grup yang dipilih harus memiliki tnkbId yang sama!",
        );
        return;
      }
    ...
    try {
      ...
      if (res.data.success) {
        notification.success({
          message: "Info",
          description: `Dokumen ${payload.itemToCancel.documentno} akan diproses untuk dicancel.`,
        });
        fetchData();
      } else {
        notification.error({
    ...
    <LayoutGlobal>
      ...
      <div style={{ marginTop: 16 }}>
        <Button
          ...
          icon={<SendOutlined />}
        >
          Handover ({getAllSelectedItems().length} Dokumen dari{" "}
          {selectedGroupRows.length} SPP)
        </Button>
      </div>
      ...
      >
        <p>
          Apakah Anda yakin akan mecancel dokumen{" "}
          <strong>{itemToCancel?.documentno}</strong>?
        </p>
        Notes:
        <TextArea
          rows={4}
