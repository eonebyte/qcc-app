...
const SupplyRawMaterial = () => {
    ...
    // FUNGSI fetchData SUDAH BENAR, TIDAK PERLU DIUBAH
    ...
    // --- FUNGSI EXPORT FINAL SESUAI GAMBAR ---
    const handleExportExcel = async () => {
        ...
        // Kelompokkan ulang data berdasarkan RM Key untuk Laporan
        ...
        // Definisikan semua header
        const mainHeaders = ['No', 'RM Key', 'RM Name', 'Tanggal', 'Supply (Kg)', 'Return (Kg)'];
        ...
        // Loop melalui data yang sudah dikelompokkan
        for (const [rmKey, rmData] of groupedByRm.entries()) {
            if (rowNumber > 1) sheet.addRow([]); // Baris pemisah
            ...
            for (let i = 0; i < loopCount; i++) {
                ...
                // Kolom Master (No, RM Key, RM Name) hanya untuk baris pertama grup
                if (i === 0) {
                ...
                // Kolom Transaksi Harian
                if (i < transactions.length) {
                    const tx = transactions[i];
                    rowData.push(tx.movementDate, tx.supplyProductionQtyKg || 0, tx.returnProductionQtyKg || 0);
                } else {
                    rowData.push('', '', '');
                }

                // Kolom Finished Goods
                ...
                // Beri style BOLD pada baris pertama grup
                if (i === 0) {
    ...
    // Bagian lain dari komponen (render, columns, dll.) tidak perlu diubah
