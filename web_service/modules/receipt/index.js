      ...
      const queryPostgres = `
              ...
              FROM (
                  SELECT
                      ...
                      ROW_NUMBER() OVER (
                          PARTITION BY t.adw_trackingsj_id
                          ORDER BY gs.adw_handover_group_id DESC
                      ) AS rn
    ...
    try {
      ...
      for (const bundle of bundles) {
        ...
        if (updateResult.rowCount === 0) {
          throw new Error(
            `Update gagal: documentno ${bundle.bundleNo} tidak ditemukan.`,
          );
        }

        console.log(
          "Update handover group berhasil. rowCount =",
        ...
        const {
          ...
          bundleCheckpoint,
          dateHandover,
        } = await server.tms.listBundleDetailPDF(dbClient, bundle.bundleNo);
        const payload = {
          listShipment,
          dataUser,
          bundleNo,
  ...
  async listDriverFromDPK(server) {
    ...
    `;
      ...
      const allTrackingIds = [
        ...new Set(
          groupRows.map((r) => r.adw_trackingsj_id).filter((id) => id !== null),
    ...
    `;
      ...
      const trackingResult = await dbClient.query(trackingQuery, [
        allTrackingIds,
      ]);
      ...
      const inoutIds = [
        ...new Set(
          trackingRows.map((t) => t.m_inout_id).filter((id) => id !== null),
        ),
      ];
    ...
    try {
      ...
      for (const bundle of bundles) {
        ...
        const getGroupIdQuery = `
                    ...
                    WHERE documentno = $1
                `;
        const groupRes = await dbClient.query(getGroupIdQuery, [
          bundle.bundleNo,
        ]);
